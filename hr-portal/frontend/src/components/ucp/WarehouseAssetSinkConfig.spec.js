...
describe('WarehouseAssetSinkConfig', () => {
    ...
    it('不能表达或试图覆盖 PK 时暴露 lossy_write_blocked 并禁止保存', async () => {
        ...
        expect(wrapper.vm.lossyWriteBlocked).toBe(true);
        expect(wrapper.vm.mappingSaveState).toMatchObject({ code: 'lossy_write_blocked', canSave: false });
    ...
    it('unsupported legacy transform 初次读取即阻断无损写回且不丢原 mapping', async () => {
        ...
        expect(wrapper.vm.mappingSaveState).toMatchObject({
