...
const document = () => ({
    ...
    ruleSet: {
        ...
        rules: [{
                id: 'r1', type: 'field', enabled: true, displayOrder: 0,
                sourceFields: ['employee_no'], targetFields: ['employee_no'], config: { mode: 'rename' },
            }],
    },
});
const stubs = {
...
describe('MappingWorkspace effects contract', () => {
    ...
    it('moves focus into the editor and returns it to the rule toggle', async () => {
        ...
        await flushPromises();
        expect(globalThis.document.activeElement.id).toBe('mapping-rule-panel-r1');
        await toggle.trigger('click');
        await flushPromises();
    ...
    it('reports whether an exposed rule focus succeeds', async () => {
        ...
        expect(await wrapper.vm.focusRule('r1')).toBe(true);
        expect(await wrapper.vm.focusRule('missing')).toBe(false);
        const readonlyWrapper = mount(MappingWorkspace, {
            props: { modelValue: document(), policy: policy({ effects: { allowSave: false } }) },
            global: { stubs },
        });
        expect(await readonlyWrapper.vm.focusRule('r1')).toBe(false);
    });
    it('exposes publish, execute and rebuild capability values to footer slot', async () => {
        const wrapper = mount(MappingWorkspace, {
            ...
            slots: { 'footer-actions': (slotProps) => `save=${slotProps.canSave};publish=${slotProps.canPublish};execute=${slotProps.canExecute};rebuild=${slotProps.canRebuild}` },
            global: { stubs },
        });
        await flushPromises();
    ...
    it('shows only the requested visible rule types', () => {
        ...
        model.ruleSet.rules.push({
            id: 'r2', type: 'reference_lookup', enabled: true, displayOrder: 1,
