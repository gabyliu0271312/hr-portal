...
describe('review question entry-mode variants', () => {
    ...
    it('renders vertical evaluation and preview card structures', async () => {
        ...
        expect(wrapper.findAll('.display-option-divider')[0].element.style.backgroundColor).toBe('rgb(240, 244, 255)');
        expect(wrapper.findAll('.display-option-divider')[1].element.style.backgroundColor).toBe('rgb(222, 224, 227)');
        ...
        expect(wrapper.findAll('.display-option-divider')[0].element.style.backgroundColor).toBe('rgb(222, 224, 227)');
        expect(wrapper.findAll('.display-option-divider')[1].element.style.backgroundColor).toBe('rgb(240, 244, 255)');
    });
    ...
    it('switches calculation rules only inside the sub-item rating branch', async () => {
        ...
        await wrapper.get('input[value="sub_items"]').setValue();
        expect(wrapper.get('input[value="none"]').element.checked).toBe(true);
        ...
        expect(wrapper.get('input[value="condition"]').element.checked).toBe(true);
    ...
    it('loads candidate questions and derives rule name and score bounds', async () => {
        const rating = { id: 1, name: '评级子题', rule_id: 11, rule_name: '五级评级', review_type: '评级', grade_participates_in_calculation: true, score_min: 1, score_max: 5 };
        const score = { id: 2, name: '评分子题', rule_id: 12, rule_name: '百分制评分', review_type: '评分', grade_participates_in_calculation: false, score_min: 0, score_max: 100 };
        const wrapper = mount(ReviewQuestionRuleAdditionalCards, {
            props: {
                entryMode: 'regular_question',
                ruleType: '评级',
    ...
    it('keeps the existing display component and adds order controls for sub-item ratings', async () => {
        ...
        expect(wrapper.findAll('.order-option-seam')[0].element.style.backgroundColor).toBe('rgb(240, 244, 255)');
        expect(wrapper.findAll('.order-option-seam')[1].element.style.backgroundColor).toBe('rgb(222, 224, 227)');
        expect(wrapper.findAll('.order-option-seam')[2].element.style.backgroundColor).toBe('rgb(240, 244, 255)');
        expect(wrapper.findAll('.order-option-seam')[3].element.style.backgroundColor).toBe('rgb(222, 224, 227)');
        await wrapper.findAll('[role="radiogroup"][aria-label="填写顺序"] .order-option')[1].trigger('click');
        expect(wrapper.findAll('.order-option-seam')[0].element.style.backgroundColor).toBe('rgb(222, 224, 227)');
        expect(wrapper.findAll('.order-option-seam')[1].element.style.backgroundColor).toBe('rgb(240, 244, 255)');
    ...
    it('renders score-range sub-question calculation variants and existing display order', async () => {
        const option = { id: '7680531263614094288', name: '子评估题--评分（在分数上下限内输入评分）', rule_id: 7680421437210954714, rule_name: '评估规则--拼分（在分数上下限内输入评分）', review_type: '评分', grade_participates_in_calculation: false, score_min: 1, score_max: 10 };
        const wrapper = mount(ReviewQuestionRuleAdditionalCards, {
            props: {
                entryMode: 'regular_question',
                ruleType: '评分',
