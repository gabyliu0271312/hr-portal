...
describe('LevelConfigEditor', () => {
  it('renders the level controls and adds an inline level', async () => {
    ...
    const levelsAfterStep = wrapper.emitted('update:levels')?.at(-1)?.[0] as ReviewRuleLevel[]
    expect(levelsAfterStep[0].value).toBe('0.1')

    await wrapper.findAll('.performance-number-input input')[0].setValue('-2')
    const levelsAfterNegative = wrapper.emitted('update:levels')?.at(-1)?.[0] as ReviewRuleLevel[]
    expect(levelsAfterNegative[0].value).toBe('0')

    await wrapper.get('.add-level-button').trigger('click')
    expect(wrapper.emitted('add')).toHaveLength(1)
    const added = wrapper.emitted('update:levels')?.at(-1)?.[0] as ReviewRuleLevel[]
  ...
  it('adds quantified score inputs while keeping value inputs', async () => {
    ...
    const updated = wrapper.emitted('update:levels')?.at(-1)?.[0] as ReviewRuleLevel[]
  ...
  it('reorders only from the drag handle and preserves stable level identity', async () => {
    ...
    rows.forEach((row, index) => {
      vi.spyOn(row.element, 'getBoundingClientRect').mockReturnValue({ top: index * 44, left: 0, right: 760, bottom: index * 44 + 32, width: 760, height: 32, x: 0, y: index * 44, toJSON: () => ({}) } as DOMRect)
    })
    ...
    const reordered = wrapper.emitted('update:levels')?.at(-1)?.[0] as ReviewRuleLevel[]
  ...
  it('caps levels at ten and reports the minimum-level delete error', async () => {
    ...
    expect((minimum.emitted('update:levels')?.at(-1)?.[0] as ReviewRuleLevel[])).toHaveLength(1)
