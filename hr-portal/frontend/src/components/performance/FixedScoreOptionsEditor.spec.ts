...
describe('FixedScoreOptionsEditor', () => {
  it('renders fixed score rows with the shared handle and validates decimal precision', async () => {
    ...
    const invalidOptions = wrapper.emitted('update:options')?.at(-1)?.[0] as typeof options
    ...
    const recoveredOptions = wrapper.emitted('update:options')?.at(-1)?.[0] as typeof options
  ...
  it('reorders variable-height rows and preserves stable option ids', async () => {
    ...
    rows.forEach((row, index) => {
      vi.spyOn(row.element, 'getBoundingClientRect').mockReturnValue({ top: index * 48, left: 0, right: 400, bottom: index * 48 + (index === 0 ? 40 : 72), width: 400, height: index === 0 ? 40 : 72, x: 0, y: index * 48, toJSON: () => ({}) } as DOMRect)
    })
    ...
    const reordered = wrapper.emitted('update:options')?.at(-1)?.[0] as typeof options
  ...
  it('shows delete only after adding a third option', async () => {
    ...
    const added = wrapper.emitted('update:options')?.at(-1)?.[0] as typeof options
