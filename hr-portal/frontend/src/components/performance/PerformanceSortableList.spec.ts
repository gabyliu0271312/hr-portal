...
function mountList() {
  return mount(PerformanceSortableList, {
    ...
    slots: {
      default: ({ item, index, dragging, over, itemStyle }: { item: typeof items[number]; index: number; dragging: boolean; over: boolean; itemStyle: Record<string, string> }) => h('div', {
...
describe('PerformanceSortableList', () => {
  ...
  it('uses the dragged row height for the placeholder and displacement', async () => {
    ...
    const tops = [0, 64, 112]
    rows.forEach((row, index) => {
      vi.spyOn(row.element, 'getBoundingClientRect').mockReturnValue({ top: tops[index], left: 0, right: 760, bottom: tops[index] + heights[index], width: 760, height: heights[index], x: 0, y: tops[index], toJSON: () => ({}) } as DOMRect)
    })
