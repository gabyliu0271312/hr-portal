...
function payload(reviewType: ReviewRulePreviewPayload['reviewType']): ReviewRulePreviewPayload {
  return {
...
describe('ReviewRulePreviewModal', () => {
  ...
  it('uses equal-width mapping segments and grows the mapping shell for additional intervals', async () => {
    ...
    const modal = document.body.querySelector('.review-rule-preview-modal') as HTMLElement
  ...
  it('positions the preview cursor inside the equal-width track', async () => {
    ...
    let cursor = document.body.querySelector('.mapping-scale__cursor') as HTMLElement
    ...
    await input.setValue('0')
    cursor = document.body.querySelector('.mapping-scale__cursor') as HTMLElement
