...
function mountQuantifiedForm(mode: 'create' | 'edit') {
  return mount(ReviewRuleForm, {
    props: {
      mode,
      modelValue: {
...
describe('ReviewRuleForm', () => {
  ...
  it('toggles the quantified score column without losing its value', async () => {
    ...
    await rating.get('.performance-switch').trigger('click')
    expect((rating.get('input[aria-label="第1个等级量化分"]').element as HTMLInputElement).value).toBe('2.5')
  })
  ...
  it('switches between score and mapping configurations', async () => {
    ...
    expect((scoreMinInput.element as HTMLInputElement).value).toBe('1.2')
    expect((scoreMaxInput.element as HTMLInputElement).value).toBe('9.9')
    ...
    expect((scoreMinInput.element as HTMLInputElement).value).toBe('5.0')
  ...
  it('blocks invalid rating preview and emits a readonly payload after recovery', async () => {
    ...
    for (const [index, input] of wrapper.findAll('input[placeholder="请输入「中文」等级代号"]').entries()) await input.setValue(['C', 'B', 'A'][index])
    ;(wrapper.vm as any).preview()
    await wrapper.vm.$nextTick()
    const payload = wrapper.emitted('preview')?.at(-1)?.[0] as any
    expect(payload.name).toBe('评级预览规则')
    expect(payload.levels.map((level: any) => level.code)).toEqual(['C', 'B', 'A'])
  })
  ...
  it('validates and emits score bounds and fixed-option preview payloads', async () => {
    ...
    ;(wrapper.vm as any).preview()
    await wrapper.vm.$nextTick()
    expect((wrapper.emitted('preview')?.at(-1)?.[0] as any).score).toMatchObject({ min: '0', max: '100' })
    ...
    ;(wrapper.vm as any).preview()
    await wrapper.vm.$nextTick()
    expect((wrapper.emitted('preview')?.at(-1)?.[0] as any).score.fixedOptions.map((item: any) => item.value)).toEqual(['20', '40'])
  })
  ...
  it('validates mapping intervals before emitting the preview payload', async () => {
    ...
    expect(payload.mapping.intervals.map((item: any) => item.code)).toEqual(['C', 'A'])
  })
  ...
  it('allows negative score bounds in the preview payload', async () => {
    ...
    ;(wrapper.vm as any).preview()
    await wrapper.vm.$nextTick()
    expect((wrapper.emitted('preview')?.at(-1)?.[0] as any).score).toMatchObject({ min: '-10', max: '100' })
  })
  ...
  it.each(['create', 'edit'] as const)('blocks %s submit and preview when quantified scores are empty', async (mode) => {
    ...
    for (const [index, input] of wrapper.findAll('input[aria-label$="量化分"]').entries()) await input.setValue(String(index + 1))
    await wrapper.get('form').trigger('submit')
    ;(wrapper.vm as any).preview()
    await wrapper.vm.$nextTick()
  ...
  it('submits rating data after level codes are filled', async () => {
    ...
    for (const input of wrapper.findAll('input[placeholder="请输入「中文」等级代号"]')) await input.setValue('A')
    await wrapper.get('form').trigger('submit')
