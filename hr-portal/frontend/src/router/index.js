...
const routes = [
    ...
    },
    {
        ...
        children: [
            ...
            { path: 'ucp/executions/:id', redirect: (to) => ({ path: `/ucp/runs/${to.params.id}` }) },
            ...
            { path: 'ucp/pipeline-designer/:templateCode', redirect: (to) => ({ path: '/ucp/pipelines/designer', query: { code: to.params.templateCode } }) },
            { path: 'ucp/pipelines/create', redirect: '/ucp/pipelines/designer' },
            { path: 'ucp/pipelines/:id/edit', redirect: (to) => ({ path: '/ucp/pipelines/designer', query: { code: to.params.id } }) },
...
router.beforeEach(async (to) => {
    ...
    const requiredCode = to.meta.requiredMenuCode;
    const code = to.meta.menuCode;
    if (requiredCode) {
        if (!menuCodes.includes(requiredCode))
            return { name: 'Home' };
    }
    else if (code) {
        if (!menuCodes.includes(code))
            return { name: 'Home' };
    }
    if (to.meta.requiresPerformanceSettingsAccess) {
        try {
            const context = await performanceApi.getAccessContext();
            if (!canManagePerformanceSettings(menuCodes, context))
                return { name: 'Home' };
        }
        catch {
            return { name: 'Home' };
