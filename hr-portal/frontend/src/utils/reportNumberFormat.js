export const DEFAULT_DISPLAY_FORMAT = {
    type: 'default',
    precision: 2,
...
export const DISPLAY_UNIT_LABELS = {
    none: '无',
    thousand: '千',
    ten_thousand: '万',
    million: '百万',
...
function roundDecimal(raw, precision, rule) {
    ...
    let increment = false;
    if (rule === 'half_up')
        increment = discarded[0] >= '5';
    if (rule === 'ceil')
        increment = !negative && /[1-9]/.test(discarded);
    if (rule === 'floor')
        increment = negative && /[1-9]/.test(discarded);
    let whole = integer;
    ...
    if (increment) {
        ...
        if (precision) {
            ...
            if (next.length > precision) {
                ...
                fractionOut = next.padStart(precision, '0');
        }
        else
            whole = addStrings(whole, '1');
    }
...
function formatGrouped(raw, useThousands) {
    const { sign, integer, fraction } = decimalParts(raw);
    const grouped = useThousands ? integer.replace(/\B(?=(\d{3})+(?!\d))/g, ',') : integer;
    return `${sign}${grouped}${fraction ? `.${fraction}` : ''}`;
}
function scaleDecimal(value, divisor, multiplier = 1) {
...
export function resolveDisplayFormat(format) {
    if (!format || format.type === 'default')
        return { ...DEFAULT_DISPLAY_FORMAT, type: 'default', unit: 'none' };
    return {
        type: format.type,
