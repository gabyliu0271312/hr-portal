const removeFromList = (values, instanceId) => (values || []).filter((value) => value !== instanceId);
export function removeReportColumnInstance(current, instanceId) {
    const dependencies = {
        columnSettings: 0,
        splitFactors: 0,
    ...
    for (const [key, setting] of Object.entries(current.columnSettings)) {
        ...
        if (factors.length)
            next.split_factors = factors;
        else
            delete next.split_factors;
    ...
    const defaultSplitRule = { ...current.defaultSplitRule, factors: defaultFactors };
    if (defaultSplitRule.factor === instanceId) {
        dependencies.defaultSplitFactors += 1;
        delete defaultSplitRule.factor;
    }
    if (!defaultSplitRule.factors?.length && !defaultSplitRule.factor)
        defaultSplitRule.enabled = false;
...
export function dependencyCount(dependencies) {
