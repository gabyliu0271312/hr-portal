...
import {
  listOdsDwdAutomationConfigs, updateOdsDwdAutomationConfig,
  getWarehouseFeatures, triggerOdsDwdSync,
  ...
  try {
    if (config.enabled) {
      ...
      await ElMessageBox.confirm(
        `开启后，每次 ${config.ods_table_label || config.ods_table_name} 同步完成将自动更新 DWD，确定？`,
        '确认开启自动化',
...
function goRecipe(tableName: string) {
  router.push({ path: '/warehouse/data-recipe', query: { table: tableName } })
}

function modeLabel(mode: string) {
  const map: Record<string, string> = { cleaning_rule: '清洗规则', passthrough: '直通更新' }
  ...
  <div class="automation-page">
    <header class="page-header">
      <div class="header-left">
        <h1>数据仓库自动化配置</h1>
        <el-tag v-if="featureEnabled" type="warning" size="small" effect="dark">试点中</el-tag>
        <el-tag v-else type="info" size="small">未启用</el-tag>
        <span style="font-size:12px;color:#909399;margin-left:8px">
          Feature Flag: warehouse.ods_dwd_automation
        </span>
      </div>
      <div class="header-actions">
        <el-button size="default" @click="load" :loading="loading">
    ...
    <!-- Feature Flag 关闭 -->
    <el-alert v-if="!featureEnabled" type="info" :closable="false" show-icon style="margin-bottom:16px">
      <template #title>
        ODS→DWD 自动化能力当前未启用
      </template>
    ...
    <div class="filter-bar">
      ...
      <el-input v-model="filterKeyword" placeholder="搜索 ODS 表名" clearable size="default" style="width:200px;margin-left:8px" />
    </div>
    ...
    <el-table :data="configs" v-loading="loading" stripe size="default" style="margin-top:12px">
      ...
      <el-table-column label="更新模式" width="100">
        <template #default="{ row }">
          <el-tag size="small" :type="row.update_mode === 'cleaning_rule' ? 'success' : row.update_mode === 'passthrough' ? 'warning' : 'info'">
            {{ modeLabel(row.update_mode) }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="最近执行" min-width="140">
        <template #default="{ row }">
          {{ row.last_execution_at?.slice(0, 16) || '-' }}
        </template>
      </el-table-column>
      <el-table-column label="操作" width="240" fixed="right">
        <template #default="{ row }">
          <el-button size="small" @click="goRecipe(row.ods_table_name)">
            <el-icon style="margin-right:2px"><Setting /></el-icon>编辑
          </el-button>
          <el-button size="small" @click="doTrigger(row)" :loading="triggering === row.ods_table_name">
            <el-icon style="margin-right:2px"><Refresh /></el-icon>手动触发
          </el-button>
          <el-button
            size="small"
