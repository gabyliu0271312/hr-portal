...
const api = costCenterMappingApi;
const mapping = mappingApi;
function stubApi() {
    for (const method of ['initialize', 'copyPrevious', 'getPeriod', 'getDwdGate', 'updateException', 'confirmDiff', 'publish', 'retryNotification']) {
        if (!vi.isMockFunction(api[method]))
            vi.spyOn(api, method);
        api[method].mockReset();
    }
    if (!vi.isMockFunction(mapping.resolvePolicy))
        vi.spyOn(mapping, 'resolvePolicy');
