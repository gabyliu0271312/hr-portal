<template>
  <div class="circuit-breaker-page">
    <div class="page-header">
      <h2>熔断与限流状态</h2>
      <p class="desc">查看/管理 UCP 资源级熔断与限流配置；资源连续失败达到阈值时自动熔断，避免雪崩；可手动重置。</p>
    </div>
    ...
    <el-tabs v-model="activeTab" type="border-card">
      ...
      <el-tab-pane label="熔断器状态" name="circuits">
        ...
        <el-table :data="circuits" v-loading="loadingCircuits" stripe size="small" empty-text="暂无熔断器记录">
          <el-table-column prop="resource_code" label="资源编码" min-width="180" />
    ...
    <el-dialog
      ...
      :title="`熔断配置 - ${currentCircuit?.resource_code || ''}`"
      width="640"
    ...
    >
      <el-form :model="configForm" label-width="140px" size="default" v-if="currentCircuit">
        <el-form-item label="启用熔断">
          ...
          <el-text size="small" type="info" class="ml-8">开启后资源连续失败达到阈值将自动熔断</el-text>
        </el-form-item>
