 ...
 *   overview:    API /ucp/systems/overview 返回的聚合数据
 *   health:      'ok' | 'warn' | 'offline' | 'unconfigured' (derived from overview)
 *
 * Emits:
 *   open:        点击卡片 (打开详情抽屉)
...
import type { SystemOverview } from '@/api/ucp'
...
const props = withDefaults(
  ...
  { resources: () => [], credentials: () => [], overview: null, health: 'unconfigured' },
)
...
const statusTag = computed(() => {
  ...
  if (cred === 'expired') return '阻断'
  if (cred === 'warning') return '待续期'
  if (cred === 'none') return '未配凭证'
  return '健康'
})
...
const statusType = computed<'success' | 'warning' | 'danger' | 'info'>(() => {
  ...
  const cred = ov.value?.credential_status
  if (cred === 'expired') return 'danger'
  if (cred === 'warning') return 'warning'
  if (cred === 'none') return 'warning'
  return 'success'
})
...
const operationalStatus = computed(() => {
  ...
  if (cred === 'expired') return '凭证已过期 · 流水线已阻断'
  if (cred === 'warning') return '即将到期 · 建议立即续期'
  if (health === 'failing') return '24h 全部失败 · 需排查'
  if (health === 'unconfigured') return '未配置凭证 · 无法执行同步'
  if (latest) {
...
const metricValue = computed(() => {
  ...
  if (rate == null) return `${sync} 次`
  return `${(rate * 100).toFixed(1)}%`
})

const metricClass = computed(() => {
  const dl = ov.value?.dead_letter_count ?? 0
  if (dl > 0) return 'metric-danger'
  const cred = ov.value?.credential_status
  if (cred === 'expired') return 'metric-danger'
  if (cred === 'warning') return 'metric-warn'
  return 'metric-ok'
})

const activeCount = computed(() => ov.value?.active_count ?? props.resources.filter((r: any) => r.status === 1).length)
