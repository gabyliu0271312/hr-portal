...
describe('PipelineTriggerConfigView', () => {
  beforeEach(() => {
    ...
    mocks.pipelineTriggers.mockResolvedValue({ items: [] }); mocks.systems.mockResolvedValue({ items: [{ id: 1, system_name: 'Feishu' }] })
  ...
  it('filters the source selector to verified, published event objects', async () => {
    ...
    await selects[1].setValue('1'); await flushPromises()
    await selects[2].setValue('2'); await flushPromises()
