...
const currentSystemLabel = computed(() => {
  if (activeTab.value === 'systems') return '数据接入'
...
function goSystems() {
  router.push({ name: 'UcpDataAccess', query: { tab: 'systems' } })
}

function openSystem(systemCode: string, subTab: TabName = 'events') {
  activeTab.value = subTab
  router.push({ name: 'UcpDataAccess', query: { tab: subTab, system: systemCode } })
}
