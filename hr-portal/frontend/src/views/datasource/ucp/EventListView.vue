<template>
  <div class="event-list">
    <div class="page-header">
      <h2>事件总线（Event Bus）</h2>
      <p class="desc">UCP 事件总线：所有外部系统推送与内部触发事件统一落库，按触发器匹配后异步派发到 Pipeline。</p>
    </div>
    ...
    <el-row :gutter="12" class="stat-row">
      ...
      <el-col :span="6">
        <el-card shadow="hover" class="stat-card">
          <div class="stat-label">成功派发</div>
          <div class="stat-value success">{{ stats.dispatchedCount }}</div>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card shadow="hover" class="stat-card">
          <div class="stat-label">未匹配</div>
          <div class="stat-value warning">{{ stats.noMatchCount }}</div>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card shadow="hover" class="stat-card">
          <div class="stat-label">失败/死信</div>
...
function goTriggers() {
  router.push({ name: 'UcpEventTrigger' })
}

function goDeadLetters() {
  router.push({ name: 'UcpDeadLetter' })
}
