  ...
  <div class="project-settings-page">
    <PageHeader :title="projectName || '新建项目'" @back="goBack">
      ...
      <template #actions>
        ...
        <button v-if="currentStep > 0" class="previous-button" type="button" @click="currentStep--">上一步</button>
        <button class="next-button" type="button" :disabled="saving" @click="nextStep">{{ currentStep === steps.length - 1 ? '完成' : '下一步' }}</button>
      </template>
    </PageHeader>
    ...
    <main class="project-settings-content">
      ...
      <section v-if="currentStep === 0" class="settings-card" aria-labelledby="project-info-title">
        <div class="card-heading"><h1 id="project-info-title">基本信息</h1><button v-if="savedView" class="card-edit-button" type="button" @click="openBasicEditor">✎ 编辑</button></div>
      ...
      <section v-if="currentStep === 0" class="settings-card" aria-labelledby="people-title">
        ...
        <div v-for="(group, groupIndex) in form.evaluator_rules.groups" :key="groupIndex" class="rule-group">
          ...
          <button class="link-button" type="button" @click="addCondition(groupIndex)">＋ 添加条件</button>
          <div v-if="groupIndex < form.evaluator_rules.groups.length - 1" class="or-divider">或</div>
        </div>
        <button class="link-button add-group" type="button" @click="addGroup">＋ 添加人员范围</button>
...
<style scoped>
.project-settings-page { position: fixed; inset: 0; z-index: 100; min-width: 1200px; min-height: 0; padding-top: 0; box-sizing: border-box; overflow: auto; background: #f5f6f7; color: #1f2329; }
.project-settings-page :deep(.full-screen-modal-header) { position: sticky; top: 0; }
.project-status-badge { padding: 4px 8px; border-radius: 4px; background: #fff1e8; color: #ff7d00; font-size: 14px; }
.edit-button,.previous-button,.next-button { position: relative; z-index: 2; height: 32px; padding: 4px 16px; border-radius: 6px; font: inherit; cursor: pointer; }.edit-button,.previous-button { border: 1px solid #d0d3d6; background: #fff; color: #1f2329; }.next-button { border: 1px solid #3370ff; background: #3370ff; color: #fff; }.next-button:disabled { opacity: .6; cursor: not-allowed; }
.project-settings-content { display: block; width: var(--performance-form-surface-width); max-width: calc(100% - 2 * var(--performance-form-surface-page-gap)); margin: var(--performance-form-surface-page-gap) auto; padding: 0; box-sizing: border-box; }.project-settings-content :deep(.settings-surface) { display: grid; gap: 16px; padding: 0; background: transparent; box-shadow: none; }.settings-card { width: 100%; box-sizing: border-box; padding: 24px 32px; border-radius: 8px; background: #fff; box-shadow: 0 1px 4px rgba(31,35,41,.05); }.settings-card.flow-card,.settings-card.time-card { padding: 0; background: transparent; box-shadow: none; }.card-heading { display: flex; align-items: center; justify-content: space-between; margin-bottom: 18px; }.card-heading h1 { margin: 0; }.card-edit-button { height: 32px; padding: 4px 16px; border: 1px solid #d0d3d6; border-radius: 6px; background: #fff; color: #1f2329; font: inherit; cursor: pointer; }.rule-group-heading :deep(.performance-icon-button),.rule-row :deep(.performance-icon-button) { --performance-icon-button-size: 24px; width: 24px; height: 24px; }.settings-card h1,.settings-card h2 { margin: 0 0 10px; font-size: 20px; line-height: 28px; }.settings-card h2 { font-size: 18px; }.hint,.muted { margin: 0 0 18px; color: #646a73; font-size: 14px; line-height: 22px; }.basic-grid,.date-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px 48px; }.basic-grid label,.date-grid label { display: grid; gap: 8px; color: #646a73; font-size: 14px; }.basic-grid label span,.date-grid label span { color: #646a73; }.basic-grid .wide { grid-column: 1 / -1; }.basic-grid input,.basic-grid textarea,.date-grid input,.template-select { box-sizing: border-box; width: 100%; padding: 7px 10px; border: 1px solid #d0d3d6; border-radius: 6px; color: #1f2329; background: #fff; font: inherit; }.basic-grid textarea { min-height: 66px; field-sizing: content; resize: none; overflow-y: hidden; }.template-select { height: 36px; }.rule-group { margin-top: 16px; padding: 16px; border-radius: 8px; background: #f5f6f7; }.rule-group-heading { display: flex; justify-content: space-between; margin-bottom: 12px; font-weight: 600; }.rule-group-heading button,.rule-row button { border: 0; background: transparent; color: #646a73; cursor: pointer; }.rule-row { display: grid; grid-template-columns: 140px 120px 1fr 32px; gap: 8px; margin-top: 8px; }.rule-row select,.rule-row input { min-width: 0; height: 32px; box-sizing: border-box; padding: 4px 8px; border: 1px solid #d0d3d6; border-radius: 6px; background: #fff; font: inherit; }.rule-row .condition-values { height: 32px; }.link-button { margin-top: 10px; padding: 0; border: 0; background: transparent; color: #3370ff; font: inherit; cursor: pointer; }.or-divider { margin: 16px -16px 0; padding-top: 12px; border-top: 1px solid #dee0e3; color: #646a73; }.add-group { margin-top: 16px; }.required { color: #f04438; }.error { margin: 12px 0 0; color: #f04438; }.flow-summary { display: grid; gap: 8px; padding: 20px; border: 1px solid #e5e6eb; border-radius: 8px; }.flow-summary span { color: #646a73; }.page-notice { position: fixed; right: 24px; bottom: 24px; z-index: 10; padding: 12px 16px; border: 1px solid #ffccc7; border-radius: 6px; background: #fff2f0; color: #f54a45; }
</style>
