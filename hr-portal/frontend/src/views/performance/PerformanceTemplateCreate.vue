<template>
  <div class="full-screen-modal">
    ...
    <main class="full-screen-modal-content">
      ...
      <PerformanceTemplateContentSettings v-else-if="currentStep === 2" ref="contentSettingsRef" :template-id="activeTemplateId" />
...
const contentSettingsRef = ref<InstanceType<typeof PerformanceTemplateContentSettings> | null>(null)
...
function validate() { const name = templateName.value.trim(); if (!name) { nameError.value = '名称为必填'; return false }; if (!isEditing.value && mockTemplateNames.includes(name)) { nameError.value = '该模板名称已存在，请重新输入'; notice.value = nameError.value; return false }; nameError.value = ''; if (calculationEnabled.value && selectedRules.value.length === 0) { calculationError.value = true; return false }; calculationError.value = false; return true }
...
async function saveTemplate() {
  if (!activeTemplateId.value || saving.value) return
  saving.value = true
  try {
...
.full-screen-modal-header-right { display: flex; align-items: center; flex: 1; height: 32px; }
.full-screen-modal-header-actions { display: flex; align-items: center; justify-content: flex-end; gap: 12px; flex: 1; height: 32px; }
...
.full-screen-modal-header .previous-button { border: .666667px solid #d0d3d6; background: #fff; color: #1f2329; }
.full-screen-modal-header .next-button { margin-left: 0; border: .666667px solid #3370ff; background: #3370ff; color: #fff; }
.full-screen-modal-content { display: flex; flex: 1 1 auto; min-width: 1200px; overflow: auto; background: #fff; }
