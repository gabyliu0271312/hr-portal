...
async def create_table(
    ...
    await db.execute(sa_text(
        f'ALTER TABLE "{dwd_table_name}" '
        'ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY'
    ))
    await db.execute(sa_text(
        f"SELECT setval(pg_get_serial_sequence('public.{dwd_table_name}', 'id'), "
        f"COALESCE((SELECT MAX(id) FROM \"{dwd_table_name}\"), 1), "
        f"(SELECT MAX(id) IS NOT NULL FROM \"{dwd_table_name}\"))"
    ))
