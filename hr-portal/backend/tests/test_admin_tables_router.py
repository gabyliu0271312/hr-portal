...
async def test_create_table_builds_empty_entity_table(monkeypatch, fake_user):
    ...
    assert "ADD GENERATED BY DEFAULT AS IDENTITY" in executed_sql
    assert "pg_get_serial_sequence" in executed_sql
    assert executed_sql.index("ADD PRIMARY KEY (id)") < executed_sql.index("ADD GENERATED BY DEFAULT AS IDENTITY") < executed_sql.index("setval(")
