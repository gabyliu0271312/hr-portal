# -*- coding: utf-8 -*-
"""X05 refresh policies: warehouse_asset_refresh_policies + warehouse_asset_refresh_runs
...
def upgrade() -> None:
    ...
    if "warehouse_asset_refresh_policies" not in tables:
        op.create_table(
    ...
    inspector = sa.inspect(conn)
    if not _unique_exists(inspector, "warehouse_asset_refresh_policies", "uq_refresh_policy_asset"):
        op.create_unique_constraint("uq_refresh_policy_asset", "warehouse_asset_refresh_policies", ["asset_type", "asset_id"])

    inspector = sa.inspect(conn)
    if "warehouse_asset_refresh_runs" not in _tables(inspector):
        op.create_table(
...
def downgrade() -> None:
    ...
    inspector = sa.inspect(conn)
    tables = _tables(inspector)
    if "warehouse_asset_refresh_runs" in tables:
        op.drop_table("warehouse_asset_refresh_runs")
    inspector = sa.inspect(conn)
    tables = _tables(inspector)
    if "warehouse_asset_refresh_policies" in tables:
