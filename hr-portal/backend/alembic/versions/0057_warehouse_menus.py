...
Revision ID: 0057_warehouse_menus
Revises: 0056_warehouse_ucp_integration
Create Date: 2026-07-04

新增菜单节点（两层级：tab → 叶子）：
  warehouse              — 数据仓库（一级 tab）
  warehouse.assets       — 数据资产（二级叶子）
  warehouse.modeling     — 数据建模（二级叶子）
  warehouse.metrics      — 指标管理（二级叶子）
  warehouse.governance   — 数据治理（二级叶子）
  warehouse.impact       — 影响分析（二级叶子）
...
revision = "0057_warehouse_menus"
down_revision = "0056_warehouse_ucp_integration"
branch_labels = None
depends_on = None

# 新菜单定义：(code, label, icon, parent_code)
NEW_MENUS = [
    ("warehouse", "数据仓库", "DataBoard", None),
    ("warehouse.assets", "数据资产", "Folder", "warehouse"),
    ("warehouse.modeling", "数据建模", "Edit", "warehouse"),
    ("warehouse.metrics", "指标管理", "TrendCharts", "warehouse"),
    ("warehouse.governance", "数据治理", "Checked", "warehouse"),
    ("warehouse.impact", "影响分析", "Connection", "warehouse"),
]
