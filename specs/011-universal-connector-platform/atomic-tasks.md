...
- [x] X0262 Object-type-first resource-template profiles
  - Contract: resource templates expose object type as the primary authoring choice. The service derives and persists `resource_defaults.configuration_profile`, then derives the internal connector and adapter; the technical resource connector type is compatibility-only and cannot be selected in the catalog UI.
  - Migration: `0165_resource_template_configuration_profiles`, `0166_resource_template_runtime_binding`, and `0167_repair_resource_template_source_schema` convert existing templates to the strict profile schema and remove `resource_connector_type`. TABLE remains one object type; future providers add a profile/adapter without adding an object type.
  - UI: users select only object type. EVENT_TYPE, REPORT and API_OBJECT derive their unique profile; TABLE derives its profile from the mutually exclusive table locator fields. The resource-template list displays object type and a read-only configuration-profile summary, not runtime implementation type.
  - Acceptance: complete required provider locators drive profile derivation; incompatible or incomplete TABLE locators and a REPORT without `report_id` are rejected by backend regression tests; template create/update persists server-owned `configuration_profile` and `runtime_binding`; frontend production build passes.
