...
| `CompletionRatePanel` | 项目管理业务组件 | 完成率节点 | `action({ action, nodeKey })` | 只发出催办/开通意图，不调用 API；未开始/未配置节点不发出操作入口 |
...
## 7.1 周期总览完成率真实数据（PM-T010-T10）

- `ProjectManagement` 使用概览接口返回的 `completion_nodes`，不在前端写入完成率、完成数或截止时间。
- 卡片显示：百分比取 `completion_rate`；完成情况显示 `completed_count/total_count`；截止时间将 `deadline_at` 转为 GMT+8，过期追加“（已截止）”。
- `status=not_started` 时仅百分比显示“未开始”；完成情况仍显示真实的 `completed_count/total_count`，使用 20px/600、`#8F959E`、系统字体；卡片不显示“去催办”或“去开通”。
- `status=unavailable` 时保留卡片结构但显示 `--`，不显示操作；真实数据为空不能回退为模拟进度。
- 聚合口径、权限范围和结果开通状态以 `api-contract.md#11-总览完成率数据` 为准。
...
- visible：三组章节“结果总览/统计详情/差异分析”；九项“绩效总览/部门统计/团队统计/序列统计/级别统计/司龄统计/自评 / 终评对比/绩效维度对比/历史绩效对比”；筛选入口；显式模拟说明；已采集结构的评级柱图及部门、团队、序列矩阵。
...
- mock展示：蓝色单系列柱图、部门/团队/序列矩阵和自评/终评交叉矩阵为设计系统预览，不宣称采集柱色/轴线或真实对比算法已还原；自评/终评矩阵按“终评行 × 自评列”展示7×7评级单元格，分母396，单元格展示占比与人数，沿用#E1EAFF/#BACEFD热力色阶及两块重点关注区域条纹。级别统计、司龄统计、绩效维度对比和历史绩效对比仍显示预留说明。矩阵保留两个评级下拉入口、评级胶囊、空值--、可点击单元格的键盘焦点和底部“添加备注（仅自己可见）”。部门统计首列为“部门”并保留汇总行，团队统计首列为“上级”并按上级层级展开且保留汇总行，序列统计首列为“序列（筛选结果含子序列）”并保留序列树与汇总行。筛选/添加对比点击给出明确“尚未接入”反馈，并仅暴露动作事件，不伪造表单或结果。
...
## 10. 级别统计真实接入（PM-T010-T08，2026-09-21）

用户明确授权本任务 P0/P1 定向豁免；完整采集与像素还原仍 blocked。UI Expert 的限定交接为 `extracted-ui-contract.json#/variant_contracts` 中 `report-level-api`，需求文字不是采集截图。

- 复用现有报表面板与矩阵，不新增表格引擎。首列“岗位职级”，汇总首行固定不参与排序；平面职级行无展开控件、无子行，零值显示 `--`。
- 评级标签与排序头复用当前组件，颜色来自模板。真实接口不提供已确认热力分档时中性显示，不猜阈值。
- 级别章节按需请求真实 API，章节内明确说明“实时岗位职级，项目快照人员”；司龄章节按需请求同一真实 API 的 `dimension=tenure`，明确说明“入职日期，按当前日期计算”，展示六行司龄桶和汇总，无展开。总览/部门/团队/序列仍为模拟预览，不把混合报表整体标成真实数据。
- 人员基线、授权范围、多项目冲突拒绝及计数口径见 API 契约第6节；此任务不实现跨项目评级合成、不回写快照、不迁移。
- 加载/错误/重试/空表独立于其他维度；403、评级体系冲突及重复成员有明确提示。周期/provider 切换立即作废所有维度旧请求，错误不回退 mock。成功来源说明与矩阵同时显示。
- 真实接口、组件测试、运行时结构与像素验收分别记录；不以构建或单测替代 UI 验收。
