...
4. ✅ 补偿金查询后的文档动作沿用同一套 LLM-first chat 基座：用户完成补偿金只读试算后，可继续在对话中要求预览或打印解除协议。能力 extractor 输出结构化 `followup_action`（如 `agreement_preview` / `agreement_print`），确定性代码只读取上一轮会话槽位、校验权限和参数完整性，然后返回通用前端动作 `document_preview` / `document_print`。前端用统一文档预览组件执行 action，并复用业务接口生成预览/PDF；不得在路由层或前端通过关键词、正则、按钮文案来判断用户意图。
