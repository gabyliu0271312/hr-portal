...
- [x] R0013 执行端到端回归和同批发布检查
  ...
  - 已完成证据：旧顶层字段全仓搜索、前后端同批发布、同步回滚资产、技术核验和业务验收记录见 [evidence-r0013-legacy-callers.md](./evidence-r0013-legacy-callers.md)。
  - 发布版本：`ai-envelope-20260719.1`，提交 `3fa04825c1d12ec516fabd5e42940053b01ec8e3`，于 2026-07-19 发布至正式环境。
...
- 发布日期：2026-07-19。
- 发布版本：`ai-envelope-20260719.1`。
- 发布提交：`3fa04825c1d12ec516fabd5e42940053b01ec8e3`。
- 发布方式：前后端以同一发布 tag 构建并同步切换；生产专用 `docker-compose.yml`、端口和 `.env` 不随应用源码覆盖。
- 回滚资产：已保留切换前源码归档、后端/前端镜像回滚标签，以及生产环境 `releases/current-ai-envelope.txt` 版本记录。
- 技术核验：后端与前端运行镜像均匹配发布 tag；部署源码与发布副本逐文件比对一致；数据库迁移已是最新；后端健康接口和前端入口可用。
- 业务验收：补偿金试算与比较、数据对比、自动化规则草稿、动作按钮、无权限提示和异常提示均已由验收方确认正常。
- 结论：R0013 完成；本次不兼容协议已完成正式发布，前后端版本一致且具备同步回滚条件。
