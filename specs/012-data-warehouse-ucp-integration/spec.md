...
```text
数据接入层：DataSource / UCP / Push/API 接入
ODS 层：RegisteredTable / 原始表 / 原始字段
DWD 层：Standardization Rule / Cleaning Rule / DWD View / Standard DataSet
DWS 层：Metric Catalog / Dimension / DWS Aggregate / Metric Materialization
ADS 层：ADS Definition / Published View / API Candidate / PushTarget Candidate
治理层：Impact Analysis / Quality Issue / Lineage / Permission / Audit
消费层：BI / API / Export / PushTarget / 业务应用
```
...
- 资产名称 / 中文名。
- 资产类型：表、视图、模型、指标、API。
- 分层：RAW/ODS/DWD/DWS/ADS/DM/METRIC。
- 主题域。
...
- **二期**：建设只读 AI-ready 元数据契约、能力注册清单、Context 安全裁剪、权限传播字段和只读解释类能力的接口预留；UI 仅允许“解释/总结/影响说明”类入口，并展示数据范围和权限提示。
- **三期**：在 ODS→DWD、DWD→DWS、DWS→ADS 的受控向导中引入“生成草稿”能力；AI 只生成规则/聚合/指标/ADS 定义草稿，不直接保存、不发布、不调度。
