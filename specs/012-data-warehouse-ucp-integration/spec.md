...
> 统一数据映射协同：公共 `MappingWorkspace` 和 adapter 见 017；本 Spec 与 `r0101-rule-scope-decision.md` 继续约束 `standardization_rules` 唯一规则正文、ODS 不回写和 DWD 派生边界。工资 `expense_type` 必须先双跑逐行比对并完成回滚演练，再从同步阶段切换到 DWD 派生。（后续阶段，不回填一期）
