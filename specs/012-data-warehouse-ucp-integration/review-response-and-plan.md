...
**日期**：2026-07-07（2026-07-08 更新：P0/P1/P2/P3/P4 实现完成，生产已部署）
**评估人**：gaby.liu + Claude
**评审对象**：R 章 Code Review 验收报告 §4-7 改进建议
**当前状态**：P0a/P0b/P0 安全层已实现，P1-P5 全链路已交付
...
- [x] Z01 Warehouse Service 按业务阶段拆分为 5 文件（assets/standardization/modeling/materialization/consumption）✅ 已完成
- [x] Z02 关键链路自动血缘边写入（标准化/SCD/快照/ADS发布/指标计算，5 处）✅ 已完成
- [x] Z03 ODS 只读保护（PostgreSQL GRANT + 应用层禁止写入校验）✅ 已完成（`layer_policy.py`）
...
1. [x] `layer_policy.py` 存在，`validate_ddl_operation` / `validate_layer_transition` / `assert_not_ods_write` 通过测试 ✅
2. [x] 全 Schema `extra="forbid"` 已覆盖 ✅
3. [x] pytest 全量通过 ✅
4. [x] DDL 破坏性操作通过真实 `registered_tables` 校验 ✅
5. [x] CREATE OR REPLACE 按 REPLACE 处理 ✅
6. [x] 前端 `npm build` 零 error ✅
7. [x] Docker 运行 + API 可达 ✅
8. [x] `router import OK` + `pytest 561 passed` ✅
...
> **2026-07-09 修正**：数据资产仅为物理表目录（RegisteredTable），不包含模型资产/指标资产/消费资产。模型已在"数据建模"模块管理，指标在"指标管理"管理，消费资产在"数据服务"管理。在资产页多开 Tab 放一份只读列表只会制造"我该去哪操作"的困惑，不建这种 UI 补丁。

```
数据仓库
├─ 数据资产              物理表目录（RegisteredTable）
├─ 数据清洗              ODS → DWD
├─ 数据建模              DWD → 模型/DWS
│   ├─ 模型设计
│   ├─ 维度管理
...
### P0a：治理红线 ✅ 已完成（2026-07-08）

> 这些是分层治理的底线约束，前端过滤 + 后端校验双保险。**已全部实现并部署生产。**
...
#### P0a-3：清洗目标表后端安全兜底 ✅ 已完成（方案已调整）

> 最终方案修正：DWD 表由 P2-01（ODS 拉取时）自动创建，清洗只更新内容。目标表名全由系统推导（`_derive_dwd_name`），用户不可覆盖。命名规范在资产创建入口强制执行。

- 后端 `execute_full`：`target_table` 参数保留但改为可选，不传时自动推导 `dwd_xxx`
- 前端：普通用户不显示目标表输入框，只读展示"将发布为 DWD 标准表 / `dwd_xxx`"
- 验收：API 直接传 `target_table=ods_xxx` → 400

> ~~原始设计（已调整）：admin 可覆盖目标表名。后经讨论确认：命名规范应在资产创建时通过 `ods_` 前缀强制执行，不在清洗模块提供覆盖入口。~~

#### P0a-4：ODS 禁止消费 ✅ 核心链路已完成，资产详情页改造列入 P1

- 报表模块：前端 `ReportDesigner` 过滤 ODS + 后端 `create_report`/`update_report` 拒绝 ODS 数据集 ✅
- 建模模块：前端 Quick/Visual 过滤 ODS + 后端 `create_model`/`publish_model` 拒绝非 DWD 输入 ✅
- 创建推送/API 时校验 `source_layer != "ODS"` ✅
- 资产详情页 `WarehouseAssetDetail.vue` ODS 隐藏 `PushTargetList`、改为跳转"数据清洗" ⏳ P1

#### P0a-5：过渡期旧模型处理 ✅ 已完成

- 新建模型：禁止 ODS 输入（P0a-2 覆盖）✅
- 已有 ODS 模型：生产环境已通过 P1 历史迁移全部转为 DWD 数据集引用，旧 ODS 数据集已删除 ✅
- `publish_model` 校验输入表层级为 DWD ✅
...
| ODS 表创建模型 | 400 | ✅ |
| DWD 表创建模型 | 成功 | ✅ |
| 旧 ODS 模型（非 DWD 输入）发布 | 400 | ✅ |
| 清洗 `target_table` 非 `dwd_` 前缀 | 400 | ✅ |
| ODS 创建推送/API/消费资产 | 400 | ✅ |
| 建模入口传 ODS 表 | 400 | ✅ |
| 新建表非 `ods_` 前缀 | 400 | ✅ |
| 报表引用 ODS 数据集 | 400 | ✅ |
| `test_admin_tables_router` 9/9 | 全部通过 | ✅ |
| `test_warehouse_models` 15/15 | 全部通过 | ✅ |

### P0b：轻量信息架构调整 ✅ 部分完成（核心菜单已完成，其余列入 P1）

> 菜单/路由基础已有。治理红线已通过。数据清洗独立菜单、建模 Tab 调整已完成；资产页多 Tab、数据服务统一入口、资产详情页消费展示改造列入 P1。

#### P0b-1：数据建模 Tab 调整 ✅ 已完成

- 文件：`WarehouseModeling.vue`
- 数据清洗已独立为顶层菜单（不再挂在"数据建模"下）

#### P0b-2：数据资产页增加资产类型 Tab ⏳ P1

- 当前资产页通过 `warehouse_layer` 筛选，DWD 表需先在 `registered_tables` 注册
- 模型资产/指标资产/消费资产 Tab 列入 P1

#### P0b-3：资产详情页消费展示修正 ⏳ P1

- ODS 资产详情页禁止消费的入口改造列入 P1

#### P0b-4："消费资产"命名说明

- 数据资产 > 消费资产（P1）：资产目录视角
- 数据服务 > 消费资产（已存在）：服务配置视角
...
| 2 | 数据建模页只显示 DWD 表 | ✅ |
| 3 | 后端拒绝 ODS 表创建模型 | ✅ |
| 4 | 数据资产页有表格资产/模型资产两个 Tab | ⏳ P1 |
| 5 | ODS 资产详情显示"禁止消费" | ⏳ P1 |
| 6 | DWD 资产详情显示"受控消费" | ⏳ P1 |
| 7 | 清洗页不显示目标表输入框 | ✅ |
| 8 | 所有建模入口（含快速关联）只显示 DWD | ✅ |
| 9 | 前端 `npm build` 零 error | ✅ |
| 10 | 后端 `pytest` 全量通过（561 passed） | ✅ |
...
| 数据资产页增加模型/指标/消费资产 Tab | 各模块已各自管理；在资产页多放一份只读列表制造困惑，不做 |

---

# 十、整合开发计划（剩余工作）

> **2026-07-09 整合**。以下仅列未实现内容。已交付项不重复：
>
> P0 安全层（`layer_policy.py`、`extra="forbid"`、嵌入式校验、权限继承摘要、血缘 5 处写入、Service 5 文件拆分）、P0a 治理红线（清洗仅 ODS、建模仅 DWD、ODS 禁止消费、清洗目标表自动推导）、菜单重组（7 个顶层菜单已调整）均已实现并部署生产。验证标准：`pytest 561 passed`、`npm build` 零 error。

## 10.1 当前基线

| 模块 | 当前实现 |
|------|----------|
| 数据资产 | `WarehouseAssets.vue` — 物理表目录，按层/类型筛选。无多余资产类型 Tab |
| 数据清洗 | `WarehouseDataRecipe.vue` — 仅 ODS 来源，目标表系统推导，不可覆盖 |
| 数据建模 | `WarehouseModeling.vue` — 仅 DWD 来源，含模型设计/维度/汇总/快照/拉链 |
| 指标管理 | `WarehouseMetrics.vue` — 指标口径定义/结果查询 |
| 数据服务 | `WarehouseDataService.vue` — 5 个 Tab 框架已存在（消费资产/API 服务/数据推送/订阅管理/服务监控），**仅"消费资产"Tab 激活**，其余显示"规划中" |
| 数据治理 | `WarehouseGovernance.vue` — 数据质量/血缘/执行监控 |
| 影响分析 | `WarehouseImpact.vue` |

## 10.2 核心设计原则

| 原则 | 说明 |
|------|------|
| 一次开发平台能力，后续前端配置服务实例 | 新建 API/推送/订阅不需要写代码；只有新渠道/新认证/新执行器才开发 |
| 各模块管各自的资产 | 模型在建模页管，指标在指标管理管，消费资产在数据服务管。不跨模块复制只读列表 |
| UCP 管入仓，仓库管生产，数据服务管出仓 | 三模块边界不混淆 |
| 消费层级红线 | ODS 禁止消费（已实现） → DWD 受控 → DWS 推荐 → ADS 标准 |

## 10.3 P1 — 数据服务功能实现（预计 4 天）

### P1-1：ServiceSourceRef 统一来源协议

**后端**：新增 `ServiceSourceRef` 数据结构，支持 `table / dataset / metric / ads / report` 五种来源类型。

```python
# 新建 backend/app/warehouse/service_ref.py
...
**兼容规则**：
- P1 新建推送/API/订阅时，保存 `source_type / source_id / source_label`
- 旧 `source_table = "report:{id}"` 仅作为读取兼容，**禁止新增此类字符串协议**
- 旧 `source_table` 为纯表名时，等价于 `source_type="table"`

**验收**：`ServiceSourceRef` 被数据服务所有端点（推送/API/订阅）统一引用。

### P1-2：数据推送 Tab 功能

**当前状态**：`WarehouseDataService.vue` 中"数据推送"Tab 显示"规划中"，不可点击。

**后端改造**（`push/router.py`）：
- `PushTargetIn.source_table` 扩展为可选，新增 `source_type / source_id / source_label` 字段（P1 阶段与 `source_table` 并存，优先用新字段）
- `PushTargetOut` 返回 `source_type / source_id / source_label`
- 查询接口支持 `?source_type=report&source_id=123` 过滤
- 来源层级校验：传入的 `source_type=table` 且为 ODS → 400

**前端改造**：
- "数据推送"Tab 从 `disabled` 改为可激活
- 嵌入 `PushTargetList` 组件，改造成支持多来源（见 P1-6）
- 列表支持按来源类型、状态、推送方式筛选
- 聚合展示所有推送：表推送、报表推送、ADS 推送

**验收**：用户可在数据服务 Tab 中创建/查看全部推送，不再只能从报表/资产详情页分散查找。

### P1-3：API 服务 Tab 功能

**页面**（`/warehouse/service` → API 服务 Tab）：

列表字段：API 名称、来源类型/名称/层级、状态（草稿/已启用/已停用/异常）、鉴权方式、今日调用量、最近调用时间、操作（详情/启停/复制地址/监控/删除）。

新建/编辑抽屉：
- 基本信息：API 名称、描述、负责人
- 来源选择：`ServiceSourceRef`（复用 `ServiceSourcePicker`）
- 字段配置：返回字段白名单、字段别名、脱敏状态（复用 `ServiceFieldSelector`）
- 查询条件：允许过滤字段、默认排序、分页上限
- 权限策略：可调用角色/用户/系统、行级权限继承策略（复用 `PermissionPolicyEditor`）
- 安全策略：限流、超时、是否允许导出
- 发布设置：是否启用、有效期

**后端**（新增 `backend/app/warehouse/api_service/`）：
- 数据模型：`api_services` 表（name/source_type/source_id/source_label/field_whitelist/filter_fields/auth_policy/rate_limit/status）
- CRUD router
- 鉴权中间件（复用现有登录态/Token 机制）
- 动态查询执行器：根据 `source_type` 路由到对应数据源取数
...
**验收**：用户在前端表单选择一个 DWD/DWS/ADS 表 → 配置字段 → 发布 → 拿到 API URL。不需要写一行后端代码。

### P1-4：订阅管理 Tab 功能

**页面**（`/warehouse/service` → 订阅管理 Tab）：

列表字段：订阅名称、来源、接收对象（人/群/系统）、频率（每天/每周/每月/事件）、格式（消息/文件/链接/API回调）、状态、最近发送时间、操作（暂停/恢复/编辑/退订/日志）。

新建/编辑抽屉：
- 来源选择：`ServiceSourceRef`
- 接收人/群/系统配置
- 订阅频率（复用 `ScheduleEditor`）
- 推送格式
- 字段范围（复用 `ServiceFieldSelector`）
- 权限确认：接收人需有来源访问权限

**后端**（新增 `backend/app/warehouse/subscription/`）：
- 数据模型：`subscriptions` 表
- 执行引擎：定时任务触发 + 事件触发
- 与 `push_service.py` 复用推送执行链

**硬约束**：ODS 禁止订阅；接收人必须通过权限校验。

### P1-5：服务监控 Tab 功能

**页面**（`/warehouse/service` → 服务监控 Tab）：

顶部指标卡：今日 API 调用量、今日推送次数、今日订阅分发次数、失败数、异常服务数。

列表：时间、服务类型（API/推送/订阅/消费资产发布）、服务名称、来源、结果、耗时、调用方、错误摘要、操作（详情/重试/跳转上游生产监控）。

筛选：时间范围、服务类型、状态、来源类型、服务名称。

**后端**：统一 `service_run_logs` 日志表，API 调用、推送执行、订阅分发统一写入。服务失败且原因为"上游数据未生产"时，返回 `upstream_failure: true` + `governance_run_id` 以便前端跳转治理执行监控。

**硬约束**：不在服务监控中重跑 ODS→DWD→DWS 生产任务；只能跳转"数据治理 > 执行监控"。

### P1-6：通用前端组件抽取

以下组件被 API 服务/数据推送/订阅管理复用，必须抽取为公共组件，**禁止三套相似表单各自复制一份**：

| 组件 | 被哪些功能复用 | 职责 |
|------|----------------|------|
| `ServiceSourcePicker` | API / 推送 / 订阅 | 选择 `table/dataset/metric/ads/report` 来源，封装 ODS 禁止逻辑 |
| `ServiceFieldSelector` | API / 推送 / 订阅 | 字段白名单、别名、脱敏状态标记 |
| `PermissionPolicyEditor` | API / 推送 / 订阅 / 消费资产 | 调用方授权、接收方校验、权限摘要展示 |
| `ScheduleEditor` | 推送 / 订阅 | 手动/定时/事件触发/cron 配置 |
| `DeliveryTargetEditor` | 推送 / 订阅 | 飞书/邮件/Webhook/文件 目标配置 |
| `ServiceRunLogPanel` | API / 推送 / 订阅 / 服务监控 | 最近执行记录、失败原因、重试入口 |
| `ServiceStatusBadge` | 所有数据服务列表 | 草稿/启用/停用/异常/下线 状态展示 |

**改造 PushTargetList**：P1 阶段将其拆分为"推送业务容器 + 通用子组件"。推送容器保持现有推送执行/启停逻辑；来源选择/字段选择/权限配置改为复用通用子组件。API/订阅复用通用子组件但不复用推送容器。

### P1-7：报表推送纳入数据服务统一管理

**原则**：报表推送属于数据服务消费体系，但不破坏现有报表用户工作流。

| 位置 | 做法 |
|------|------|
| 报表设计器推送 Tab | **保留**，定位为"当前报表推送快捷配置" |
| 报表列表"推送"按钮 | **保留**，定位为"立即执行当前报表推送" |
| 数据服务"数据推送"Tab | **新增**聚合展示报表推送，来源类型识别为 `report` |
| 跳转链接 | 在报表推送区域增加"在数据服务中统一管理"链接，跳转 `/warehouse/service?source_type=report&source_id={id}` |

**权限**：数据服务展示报表推送时，**必须叠加报表可编辑权限**（`_ensure_report_push_editable`）。不能被 `warehouse.service` 权限单独绕过。

### P1-8：数据服务权限归口

| 端点 | 当前权限 | 改为 |
|------|----------|------|
| ADS 端点 | `warehouse.modeling` | `warehouse.service` |
| PushTarget 非报表推送 | `system.users`（不合理） | `warehouse.service` |
| 报表推送 | 报表可编辑校验 | `warehouse.service` + 报表可编辑校验（双叠加） |
...
| 层级 | 当前状态 | 改造后 |
|------|----------|--------|
| ODS | 无明确消费拒绝提示 | 隐藏推送/API/消费入口，显示"去数据清洗 →"跳转 `/warehouse/data-recipe` |
| DWD | 无分层差异化提示 | 显示"创建数据服务"跳转 `/warehouse/service?source_type=table&source_id={table_name}`，提示需配置脱敏和权限 |
| DWS | 同上 | "创建数据服务"跳转，推荐创建 API/推送 |
| ADS | 同上 | "管理消费服务"跳转数据服务 |

**详情页拆分为两个摘要区**：
1. **数据连接摘要**：UCP 资源、DataSource 入仓来源、同步记录、跳转 UCP/DataSource
2. **数据服务摘要**：API 数量、推送数量、服务监控状态、跳转数据服务

禁止在资产详情页扩展完整连接器配置或完整推送配置表单。

### P1-10：UCP/DataSource/数据服务边界强化

**一句话边界**：UCP 管数据怎么进来，数据仓库管数据怎么生产，数据服务管数据怎么出去。

| 模块 | 负责 | 不负责 |
|------|------|--------|
| UCP / DataSource | 外部连接、凭证、连接测试、Pipeline、入仓同步 | 仓内建模、ADS 发布、出仓 API/推送 |
| 数据仓库 | 分层加工、清洗、建模、汇总、快照、拉链、质量、血缘 | 外部连接器、凭证明文、跨系统 Pipeline |
| 数据服务 | 仓内资产和报表的 API、推送、订阅、服务监控 | 外部数据采集、连接器配置、UCP Pipeline |

**代码落地**：在 `WarehouseAssetDetail.vue` 中按 P1-9 的两个摘要区分隔展示，不让用户在同一页面混淆"数据怎么进来"和"数据怎么出去"。

## 10.4 P2 — PushTarget 来源模型重构（预计 1 天）

> P2 不与 P1 并行。P1 中 PushTarget 通过新增可选字段兼容多来源；P2 完成底层迁移。

### P2-1：数据库 migration

```sql
ALTER TABLE push_targets ADD COLUMN source_type VARCHAR(16);
ALTER TABLE push_targets ADD COLUMN source_id VARCHAR(64);
ALTER TABLE push_targets ADD COLUMN source_label VARCHAR(128);
```

数据回填策略：
- `source_table` 为纯表名 → `source_type='table', source_id=source_table`
- `source_table` 为 `report:{id}` → `source_type='report', source_id='{id}'`
- `source_table` 为空 → 跳过

### P2-2：代码迁移

- `push/router.py` 写入路径：先写新字段，再同步写旧字段（兼容期）
- `push/models.py`：`source_table` 标记 deprecated，不删除列
- 前端 `PushTargetList` 统一用 `sourceType/sourceId`
- 全量回归测试

**迁移后硬约束**：禁止新增 `report:{id}`、`dataset:{id}` 等字符串协议。

## 10.5 P1 验收标准

| # | 标准 | 验证方式 |
|---|------|----------|
| 1 | 数据服务 5 个 Tab 均可激活，不再显示"规划中" | 点击每个 Tab |
| 2 | API 服务：选 DWD 表，配字段，发布，拿到 URL 可调用 | 端到端 |
| 3 | API 服务：用 ODS 创建 → 400 | API 测试 |
| 4 | 数据推送 Tab 聚合展示表推送 + 报表推送 | 查看推送列表 |
| 5 | 订阅管理：新建/编辑/暂停/退订 完整可用 | 端到端 |
| 6 | 服务监控：API 调用/推送执行/订阅分发 有日志 | 执行后查看监控 |
| 7 | 报表推送在数据服务可见，且受报表编辑权限约束 | 非创建人查看 → 403 |
| 8 | 报表设计器推送入口保留，有"在数据服务统一管理"链接 | 打开报表设计器推送 Tab |
| 9 | ODS 资产详情无消费/API/推送入口，有"去数据清洗"跳转 | 打开 ODS 表详情 |
| 10 | DWD/DWS/ADS 资产详情有"创建数据服务"跳转 | 打开对应详情页 |
| 11 | 资产详情页区分"数据连接摘要"和"数据服务摘要" | 打开详情页 |
| 12 | 7 个通用组件全部抽取，API/推送/订阅无重复表单代码 | 检查组件目录 |
| 13 | PushTarget 写入同时支持新字段和旧字段 | API 测试 |
| 14 | `npm build` 零 error，`pytest` 全量通过 | CI |

## 10.6 仍然不做

| 项目 | 理由 |
|------|------|
| 数据资产页增加模型/指标/消费资产 Tab | 各模块已各自管理，不建 UI 补丁 |
| 底层统一 RegisteredTable + DataSet | P2 架构重构，涉及大面积 breaking change |
| API 服务返回 `*` 全字段 | 安全红线，必须走字段白名单 |
| 在服务监控中重跑生产任务 | 生产/消费链路分离，跳转治理执行监控 |
| 数据服务内嵌 UCP 连接器/凭证配置 | 入仓归 UCP，不混入出仓模块 |
