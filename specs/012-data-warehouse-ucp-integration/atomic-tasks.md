...
- [x] K0801 统一报表列表、查看与设计工作区交互。
  - 前置任务：现有报表列表、运行与设计能力可用。
  - 功能范围：报表列表隐藏无子菜单侧栏并采用即时筛选、分页和精简行操作；查看和新建/编辑页复用业务无关的全屏工作区 Head。不修改报表 API、运行逻辑、权限模型或数据结果。
  - 代码交付物：公共全屏工作区组件、报表列表/查看/设计页面、路由元数据及聚焦前端测试。
  - UI 要求：关联 `ui-interaction.md` 的可用性要求、N12 与 U28；覆盖 normal/loading/empty/error/forbidden，危险操作保留确认，宽表优先获得横向空间。
  - UCP 协同要求：不涉及 UCP。
  - 测试要求：公共 Shell 组件测试、报表列表交互测试、前端 build；E2E 不适用，原因是本任务不改变后端报表契约且仓库未提供报表 E2E 基线。
  - 验收标准：列表筛选无需查询/重置按钮，快速输入不会被旧请求覆盖；筛选和页码可恢复；查看/设计页固定 Head 与返回路径一致；无权限按钮不显示。
  - 完成定义：开发、测试、构建及 N1201-N1206 合规检查全部完成。

## K09 报表访问审计

- [x] K0901 在现有操作日志中增加报表访问分类并建立敏感数据访问追踪。
  - 前置任务：K0801、现有 `SystemLog` 与 `/system-logs` API 可用。
  - 功能范围：不新增菜单或路由；复用 `系统设置 → 操作日志`，增加 `report_access` 分类；审计报表新建、修改、删除、数据查看、CSV/XLSX 导出、推送及失败/拒绝事件；精简报表列表运行统计列。
  - 代码交付物：报表审计服务、报表端点埋点、系统日志筛选/分页 Schema、操作日志报表访问视图、报表列表调整及测试。
  - UI 要求：关联 N13 与 U29；公共列表保持时间、类型、操作人、动作、对象、状态、摘要、详情结构；报表详情抽屉展示字段清单、筛选摘要、权限范围、行数、Hash、Trace 与客户端信息；不显示原始数据行或敏感筛选值。
  - 权限要求：`report_access` 复用 `system.logs.operation` 查看权限；报表业务动作继续复用 `report.list` 的 V/C/U/D/E；审计接口不得成为报表字段或数据权限旁路。
  - 数据安全：审计独立事务；保存操作人快照、报表/数据集快照、字段清单和筛选 HMAC，不保存实际数据行；删除报表后日志仍保留；审计列表无修改/删除能力。
  - 测试要求：后端单元/API 测试覆盖分类权限、筛选分页、成功/失败审计与敏感值不落日志；前端组件测试覆盖分类切换、自动筛选、详情抽屉；执行前端 build。
  - 验收标准：可按时间、操作人、动作、报表和状态追踪访问；能够回答谁在何时查看/导出了哪张报表、哪些字段和多少行；不能从日志还原业务数据行。
  - 验收证据：`test_report_access_audit.py` 5 个审计安全、失败状态、权限与分页用例通过；报表运行、字段校验、质量门禁及导出相关回归共 83 个用例通过；`OperationLogs.spec.ts` 与 `ReportList.spec.ts` 共 4 个用例通过；`npm.cmd run build` 通过。实现未新增菜单或路由，筛选值与完整配置均使用密钥 HMAC，日志不保存结果数据行。
  - 完成定义：开发、测试、构建及 N1301-N1306 全部完成后方可勾选。

## K10 表格归集列表与工作区导航

- [x] K1001 统一表格归集模板列表与工具中心布局，并将编辑类工作区改为独立路由。
  - 前置任务：现有 `TableMerge.vue`、`TableMergeTemplateCard.vue` 与 `TableToolFullscreenShell.vue` 可用。
  - 功能范围：模板列表保留应用顶部栏和左侧导航，采用与工具中心一致的标题、可见说明、卡片网格，右侧提供“新建模板”；新建、编辑、新增映射和执行合并使用独立隐藏路由与全屏工作区；不修改表格归集 API、权限、映射、派生字段或合并逻辑。
  - 代码交付物：表格归集路由记录、路由驱动的页面状态、模板列表页面外壳、未保存导航守卫及聚焦前端测试。
  - UI 要求：关联 N1501-N1506 与 U30；列表页不得显示全屏返回按钮；工作区返回一次即到模板列表；浏览器返回与 Head 返回遵循同一未保存确认；列表覆盖 loading、empty、error、forbidden 和 normal。
  - 权限要求：继续复用 `table_tools` 的 V/C/U/D/E 权限，隐藏路由不得成为权限旁路；“新建模板”仅在 C 权限下显示。
  - UCP 协同要求：不涉及 UCP。
  - 测试要求：组件测试覆盖普通列表外壳、可见说明、新建/编辑路由、工作区返回和未保存拦截；路由测试覆盖列表保留侧栏、详情隐藏侧栏；执行 `vue-tsc` 与生产 build。
  - 验收标准：从模板编辑点击一次返回即回到表格归集模板列表；模板列表与工具中心具有相同页面层级和网格节奏；刷新详情路由可恢复对应模板；无权限和 API 失败不出现空白页面。
  - 验收证据：`TableMergeNavigation.spec.ts`、`TableMergeRoutes.spec.ts`、`TableMergeTemplateCard.spec.ts`、`TableToolFullscreenShell.spec.ts` 共 9 个用例通过，覆盖普通列表、可见说明、编辑路由、一次返回、未完成步骤确认、空态、错误重试和路由侧栏元数据；`vue-tsc --noEmit` 0 错误；`npm.cmd run build` 通过（2721 modules）。
  - 完成定义：开发、测试、构建及 N1501-N1506 全部完成后方可勾选。

- [x] K1002 修复生产构建误加载旧 JavaScript 路由导致表格归集按钮失效。
  - 前置任务：K1001。
  - 功能范围：移除与 `router/index.ts` 重复且已过期的 `router/index.js`，确保应用入口和测试只解析 TypeScript 路由；不修改表格归集 API、权限或业务处理逻辑。
  - 代码交付物：删除旧路由副本，路由回归测试改为从应用真实入口 `@/router` 导入。
  - UI 要求：关联 N1507 与 U30；新建模板、模板卡片、合并、新增和删除按钮维持既有布局，修复其点击后的实际响应。
  - 权限要求：继续复用 `table_tools` 的 V/C/U/D/E 权限，不改变按钮可见性和后端门禁。
  - UCP 协同要求：不涉及 UCP。
  - 测试要求：路由测试覆盖生产入口可解析四个工作区命名路由；执行表格归集定向测试、`vue-tsc` 与生产 build。
  - 验收标准：生产构建只使用 `router/index.ts`；列表按钮能够进入对应工作区或执行既有操作，不再出现命名路由不存在导致的静默无响应。
  - 验收证据：删除旧 `router/index.js` 后，`TableMergeRoutes.spec.ts` 改从 `@/router` 导入应用真实入口；表格归集 4 个测试文件共 9 个用例通过，独立 `vue-tsc --noEmit` 通过，`npm.cmd run build` 通过（2721 modules）。
  - 完成定义：删除旧路由副本、测试和构建全部通过并完成 N1507 后方可勾选。

- [x] K1003 稳定表格归集直接映射与跨来源标准字段派生的共存计算。
  - 前置任务：K1001。
  - 功能范围：来源映射内部继续聚合和派生；跨来源汇总后，对没有直接映射目标值的人员使用已聚合标准字段补算派生公式；不兼容源列名公式，不修改模板配置、API 或 UI。
  - 代码交付物：表格归集引擎的直接值来源保护与全局派生补算，以及“个人基本医疗保险”双来源回归测试。
  - UI 要求：不涉及 UI；预览和下载继续使用同一合并结果。
  - 权限要求：不改变 `table_tools` 权限。
  - UCP 协同要求：不涉及 UCP。
  - 测试要求：覆盖派生字段两个标准依赖来自不同映射、目标字段直接映射、同人直接与派生并存、空依赖按零计算及既有引擎回归。
  - 验收标准：直接映射值不被其他映射公式覆盖；派生目标没有直接值时，可用跨来源聚合后的标准字段计算；预览与下载结果一致。
  - 验收证据：`test_table_merge_capability.py` 新增“个人基本医疗保险”跨映射标准字段补算用例；表格归集引擎、映射接口、结果列共 39 个后端用例通过；`git diff --check` 通过。当前后端虚拟环境未安装 `ruff`，未执行该静态检查。
  - 完成定义：新增回归用例与表格归集后端测试通过后方可勾选。
...
- [x] N1201 报表列表在没有有效子菜单时隐藏左侧栏，并保留全局顶部导航。
- [x] N1202 报表查看与新建/编辑使用统一全屏工作区 Head，返回路径均指向报表列表。
- [x] N1203 列表工具栏为左侧即时筛选、右侧新建报表，控件等高且不重复显示“报表管理”标题。
- [x] N1204 名称输入采用防抖搜索，数据集变化立即筛选，并防止旧请求覆盖新结果。
- [x] N1205 列表数量位于分页区，查看入口不重复，次要操作收纳且删除保持二次确认。
- [x] N1206 报表列表覆盖 loading、empty、filtered-empty、error、forbidden，查看工作区覆盖加载与运行失败反馈。
  - 关联任务：K0801。
  - UI 示意图：见 U28 报表消费工作区线框图。
  - 测试要求：组件测试验证 Shell、即时筛选、分页和错误态；执行前端 build；交付说明记录未自动化覆盖的视觉走查项。
  - 验收证据：`FullscreenWorkspaceShell.spec.ts`、`TableToolFullscreenShell.spec.ts`、`ReportList.spec.ts` 共 4 个用例通过；`npm.cmd run build` 通过。路由元数据证明列表隐藏侧栏，公共 Shell 和权限组件分别证明全屏 Head 与 forbidden 操作隐藏；empty/filtered-empty/error 文案由页面模板和组件测试覆盖。未自动化项为真实生产数据下的宽表视觉走查。
  - 完成定义：N1201-N1206 均有代码或测试证据后方可勾选。

## N13 报表访问审计 UI 原子检查

- [x] N1301 不新增日志菜单或路由，在现有操作日志类型中增加“报表访问”。
- [x] N1302 操作日志支持日志类型、时间范围、操作人、动作、对象关键词和状态筛选，并使用后端分页。
- [x] N1303 报表访问列表展示时间、操作人、动作、报表、字段数、行数、格式、状态和详情入口。
- [x] N1304 详情抽屉展示字段 Label/编码、敏感标记、筛选摘要、权限范围、配置/结果 Hash、Trace 和客户端信息。
- [x] N1305 前端不展示原始报表数据行、敏感筛选值或可用于绕过权限的查询参数。
- [x] N1306 报表列表移除运行次数、上次运行和更新时间独立列，更新时间仅作为辅助信息保留。
  - 关联任务：K0901。
  - UI 示意图：见 U29 操作日志报表访问线框图。
  - 测试要求：组件测试与 build；文字化验收覆盖 normal/loading/empty/error/forbidden 和详情状态。
  - 验收证据：操作日志复用 `/system/logs/operation` 与 `system.logs.operation`，组件测试覆盖 normal、分类切换、筛选防抖和详情；页面实现覆盖 loading、empty、error/retry，forbidden 由既有路由权限控制；生产 build 通过。
  - 完成定义：N1301-N1306 均有代码或测试证据后方可勾选。

## N15 表格归集导航与列表 UI 原子检查

- [x] N1501 表格归集模板列表使用普通内容页，保留应用顶部栏和左侧导航，不使用全屏 Head。
- [x] N1502 标题与说明文字直接可见，右侧仅放置有权限的“新建模板”主要操作。
- [x] N1503 模板网格与工具中心统一最小列宽、间距、图标和文字层级，模板业务操作仍由模板卡片承担。
- [x] N1504 新建、编辑、新增映射和执行合并使用独立隐藏路由及全屏 Head，返回一次到模板列表。
- [x] N1505 Head 返回、浏览器返回和直接路由切换共享未保存保护；刷新详情路由可恢复目标模板。
- [x] N1506 列表覆盖 loading、empty、error、forbidden、normal，工作区覆盖详情加载失败并保留 `table_tools` 权限门禁。
  - 关联任务：K1001。
  - UI 示意图：见 U30 表格归集列表与工作区导航线框图。
  - 测试要求：组件/路由测试与前端 build；文字化验收记录未自动化视觉走查项。
  - 验收证据：组件测试验证列表不渲染全屏 Shell、说明文字直接可见、模板卡进入编辑路由、一次返回和未保存确认；路由测试验证列表保留侧栏、四个工作区隐藏侧栏并复用 `table_tools` 门禁；空态和 API 错误重试有页面断言。未自动化项为生产权限账号下的真实卡片文本溢出与窄屏视觉走查。
  - 完成定义：N1501-N1506 均有代码或测试证据后方可勾选。

- [x] N1507 应用真实路由入口必须包含表格归集四个工作区命名路由，禁止保留会优先于 TypeScript 路由被解析的同名 JavaScript 副本。
  - 关联任务：K1002。
  - UI 示意图：见 U30 表格归集列表与工作区导航线框图。
  - 测试要求：从 `@/router` 导入应用实际路由并断言四个工作区路由、侧栏元数据和 `table_tools` 门禁。
  - 验收证据：应用真实路由入口测试、表格归集组件回归、类型检查与生产 build 全部通过；生产源码仅保留 `router/index.ts`。
  - 完成定义：生产入口解析路径、定向测试、类型检查和生产 build 全部通过。
...
- [x] R0001 三期启动前重新评估边界
  ...
  - 交付物：三期启动评审记录（R01-R07 各任务均声明了 UCP/数据仓库边界要求，不编辑 UCP 凭证/Pipeline）。
  - UI 要求：不涉及 UI。
  - UCP / 数据仓库边界要求：ODS→DWD 标准化加工在仓内，跨系统拉取仍归 DataSource/UCP；ADS 只组装仓内资产不直接创建外部连接器。
  - 测试要求：文档评审任务不需自动化测试。
  - 验收标准：每项三期能力（R01-R07）均已在 spec 中声明”为什么不在 UCP/帆软做”。
  - 完成定义：评审通过后才可开发 R 章后续任务。

- [x] R0002 三期分层流转主线确认
  - 任务：在开发任何 R01-R07 功能前，确认本期要支持的 ODS→DWD、DWD→DWS、DWS→ADS 流转路径、输入资产、输出资产和不可做项。
  - 交付物：
    - ODS→DWD（R01）：标准化规则 + DWD 视图生成
    - DWD→DWS（R03）：指标计算 + 维度管理 + DWS 聚合定义
    - DWS→ADS（R07）：消费资产组装 + 发布（资产/视图/API/推送候选）
    - 快照管理（R04）和 SCD 拉链（R0403）为辅助能力
    - 质量规则（R03）和调度重跑（R05）为平台公共能力
  - UI 要求：分层流转通过 WarehouseModeling 页 Tab 统一入口，每个 Tab 对应一个流转层级。
  - UCP / 数据仓库边界要求：所有 R 章能力只处理已入仓资产，不编辑 UCP 凭证、连接器或跨系统 Pipeline。
  - 测试要求：分层流转核心约束通过 pytest 38 测试。
...
查看 / 新建 / 编辑（全屏工作区）
┌──────────────────────────────────────────────────────────────┐
│ ← 返回  页面/报表名称 · 辅助摘要       刷新/预览/导出/保存 │
├──────────────────────────────────────────────────────────────┤
│                       报表工作内容                           │
└──────────────────────────────────────────────────────────────┘
```

- 列表名称输入 `300ms` 防抖，清空时立即恢复；数据集切换立即加载。
- 列表筛选和页码进入 URL，查看/编辑通过受限 `returnTo` 返回原上下文。
- normal/loading/empty/filtered-empty/error/forbidden 均需有明确表现。
- 报表名称承担查看入口；编辑为行内主要动作，复制/推送/删除进入更多菜单。
- 删除必须二次确认；无权限操作不显示。
...
系统设置 → 操作日志
┌──────────────────────────────────────────────────────────────────────────────┐
│ [报表访问 v] [时间范围] [操作人] [动作] [报表关键词] [状态]               │
├──────────────────────────────────────────────────────────────────────────────┤
│ 时间       操作人   动作       报表       字段数 行数 格式 状态      详情  │
│ 10:20:31   张三     导出Excel  工资汇总表   18   320 XLSX 成功      [查看] │
├──────────────────────────────────────────────────────────────────────────────┤
│ 共 128 条                                                   [<] 1 2 3 [>]   │
└──────────────────────────────────────────────────────────────────────────────┘

详情抽屉：报表/数据集快照、字段清单、筛选字段与 HMAC、权限范围、行数、
配置/结果 Hash、Trace、IP/User-Agent、失败原因。禁止显示实际数据行和原始敏感筛选值。
```

## U30 表格归集列表与工作区导航线框图

```text
提效工具 > 表格归集（普通内容页，保留应用导航）
┌──────────────────────────────────────────────────────────────────────┐
│ 表格归集                                            [＋ 新建模板]   │
...
新建 / 编辑 / 新增映射 / 执行合并（独立路由、全屏工作区）
┌──────────────────────────────────────────────────────────────────────┐
│ ← 返回   模板/工作区名称             步骤节点             页面操作 │
├──────────────────────────────────────────────────────────────────────┤
│ 工作区内容                                                           │
└──────────────────────────────────────────────────────────────────────┘

返回规则：工作区 Head 返回或浏览器返回均只退到 `/tools/table-merge`；
有未保存内容时先确认，刷新详情路由按 URL 恢复目标模板。模板列表不再提供第二层返回按钮。
```
