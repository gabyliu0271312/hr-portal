...
- [ ] B0510 成本中心作为 Warehouse caller 配置默认自映射、少量例外和生效范围；标准属性 Lookup 为可选组合，不固定引用 `cost_center_tree`。
- [ ] B0511 通用 binding 按生效范围解析已发布规则版本；不复制上月规则、不以 ODS 快照生成映射草稿；并发 409、DWD 门禁和通知由 caller 复用。
- [ ] B0512 未命中已发布版本或版本未发布返回 `review_required`，不写错误 DWD；ODS 本地字段复制/默认值/树构建与映射版本隔离。
