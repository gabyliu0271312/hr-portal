...
7. 支持成本中心默认自映射、少量例外覆盖、周期快照、复制上月、差异识别、人工维护、发布、DWD 门禁和通知。
...
| 成本中心维护人 | 默认自映射，仅维护例外、复制上月、发布 | DWD 和流程复用标准成本中心 |
...
使用默认源编码透传、少量例外覆盖、按标准编码 Lookup 标准名称/组织属性。周期初始化、复制上月、差异识别、人工维护、发布、审计、通知和 DWD 门禁保留为规则集业务生命周期，不新增独立映射模块。
...
8. 成本中心周期和例外属于业务生命周期数据，不要求其他调用方共用该业务表。
...
- 100 个成本中心默认自映射时只维护少量例外，DWD 结果正确。
